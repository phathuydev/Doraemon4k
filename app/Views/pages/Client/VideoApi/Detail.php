  ...
  <section class="trending-podcast-section container-video mt-5" style="margin-bottom: 180px;">
    <div class="row m-0">
      <div class="col-lg-8 col-sm-12 mt-2">
        ...
        <div class="mt-3 mb-2 ms-1 me-1 ms-md-0 me-md-0 ms-lg-0 me-lg-0 text-center text-lg-start">
          <?php foreach ($episodes as $items) {
            if (isset($items['slug']) && $items['slug'] !== 'full') : ?>
              <a href="<?= _WEB_ROOT ?>/videoApiDetail?vdId=<?= isset($_GET['vdId']) ? $_GET['vdId'] : '' ?>&slug=<?= isset($_GET['slug']) ? $_GET['slug'] : '' ?>&epi=<?= $items['slug'] ?>" class="btn btn-dark text-white me-1 mb-2 <?= (isset($_GET['epi']) && $_GET['epi'] == $items['slug']) ? 'selected' : ''; ?>">Tập <?= $items['slug'] ?></a>
          <?php endif;
          } ?>
        </div>
      </div>
